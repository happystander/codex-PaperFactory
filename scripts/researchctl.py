...
from paperfactory_core import control, evidence, gpu, interventions, memory as memory_core, network, runtime_env, task_queue, ui_config, workflow
...
ROUTE_DECISIONS = memory_core.ROUTE_DECISIONS
...
def refresh_memory(root: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    return memory_core.refresh_memory(
        root,
        state,
        phase,
...
def route_decision_for(root: Path, phase: Phase) -> dict[str, Any] | None:
    ...
    return memory_core.normalize_route_decision(raw)
...
def build_next_prompt(root: Path, state: dict[str, Any]) -> str:
    ...
    runtime_text = runtime_env.prompt_context()
    network_text = network.prompt_context()
