...
For the paper-reading to method-design handoff, survey now ranks papers explicitly in `literature/paper_priority_scores.json`, extracts paper claims and protocols into `literature/claim_extraction.json`, and maps reference repository interfaces in `literature/code_interface_map.md`. Method design then compares against the nearest three prior works, generates multiple candidate methods, runs a critic pass, and writes `method/novelty_risk.json`. A high novelty risk, shallow combination, parameter-tuning-only gap, or missing falsifying ablation blocks advancement to `method_smoke`.

For LLM, Agent, RAG, fine-tuning, preference optimization, or RL projects, survey and method phases must also apply `llm-rl-toolkit`: use Open-LLM as a resource map, write `.research/llm_tooling/tool_decision.md`, and justify why TRL, verl, ms-swift, LLaMA-Factory, OpenRLHF, or another mature framework is reused or rejected before custom infrastructure is written.
